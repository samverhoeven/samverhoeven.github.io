...
controllers.controller("teamCtrl", function ($scope, $http, $routeParams, $rootScope) {
    ...
    $http({//wedstrijden laden
        ...
        for (i = 0; i < $scope.prev5Fixtures.length; i++) {//bepalen welke ploeg gewonnen heeft
            ...
            /* bepalen of team van huidige pagina vrloren of gewonen heeft
             * en achtergrond kleur daar naar aanpassen */
            if ($scope.prev5Fixtures[i].winner == $scope.prev5Fixtures[i].homeTeamName) {
                if (homeTeamId[1] == param) {
                    $scope.prev5Fixtures[i].colorStyle = {"background-color" : "green"};
                }else{
                    $scope.prev5Fixtures[i].colorStyle = {"background-color" : "red"};
                }
            }else if($scope.prev5Fixtures[i].winner == $scope.prev5Fixtures[i].awayTeamName){
                if (awayTeamId[1] == param) {
                    $scope.prev5Fixtures[i].colorStyle = {"background-color" : "green"};
                }else{
                    $scope.prev5Fixtures[i].colorStyle = {"background-color" : "red"};
                }
            }else{
                $scope.prev5Fixtures[i].colorStyle = {"background-color" : "yellow"};
            }
        }
    }).error(function (error) {
